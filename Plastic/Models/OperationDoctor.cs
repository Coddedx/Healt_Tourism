...
namespace Plastic.Models
{
	public class OperationDoctor :BaseEntity // BİR OPERASYANU BİRDEN FAZLA DOKTOR YAPABİLİR BİR DOKTOR BİRDEN FAZLA OPERASYON YAPABİLİR
		//???????????????????? Doktor Klinikten ayrılabilir ama o operasyonları başka doktor yapabilir o zaman operasyonları doktora bağlamamak mı gerekiyor????????????????????????????????????????????????  -->>>> franchise a bağlamalıyız?????????  --> ya da management panel de işlemi yapan doktoru da değiştirebilmeliyiz???????????????
	{
		[Required]
